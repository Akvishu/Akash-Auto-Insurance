       	...
       	<div class='container p-4'>
    		...
    		%>	
    			<div class='bg-light p-3' >
    				...
    				%>
    				<img src='GetPhoto?email=<%=c_email%>&type=company' height='45'/>
    				Company Name: <b><%=company_details.get("name") %></b>
    				Company Phone: <b><%=company_details.get("phone") %></b>
    				Company Email: <b><%=c_email %></b>
    				</p>
    				<p>
    				Policy ID: <b><%=policy.get("pid") %></b><br>
    				Policy Name: <b><%=policy.get("p_name") %></b><br>
    				Sum Insured Amount: <b><%=policy.get("p_si_amt") %></b><br>
    				Description: <b><%=policy.get("p_desc") %></b>
    				<form action='BuyPolicy.jsp' method='post'>
    					<input type="hidden" name="pid" value="<%=policy.get("pid") %>"/>
    					<button class='btn btn-success' type='submit'>Buy Now</button><hr>
    				</form>
    				</p>
    			</div>
    		<%	
